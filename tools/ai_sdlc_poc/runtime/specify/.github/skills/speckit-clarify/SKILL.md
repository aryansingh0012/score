...
2. **IF EXISTS**: Load `.specify/memory/constitution.md` for project principles and governance constraints.
   ...
   **Lifecycle context signals (when available):**
   - `tools/ai_sdlc_poc/README.md` for PoC orchestration and evidence expectations
   - `tools/ai_sdlc_poc/tooling.lock.json` for pinned SpecKit/APM versions
   - `README.md` at lifecycle repo root for component boundaries and Bazel-first workflows
   - `Cargo.toml` at lifecycle repo root for Rust workspace boundaries
...
3. Load the current spec file. Perform a structured ambiguity & coverage scan using this taxonomy. For each category, mark status: Clear / Partial / Missing. Produce an internal coverage map used for prioritization (do not output raw map unless no questions will be asked).
   ...
   Lifecycle Repository Alignment:
   - Domain ownership boundaries: Python PoC (`tools/ai_sdlc_poc`) vs lifecycle product components (`score/launch_manager`, `score/health_monitor`) vs Rust crates
   - Cross-language impact declaration: whether spec changes affect C++, Rust, Python, or multiple domains
   - Validation pathway expectations: when pytest-only is sufficient vs when Bazel/Cargo validation is required
   - Evidence/provenance obligations: lockfile alignment, dr009/dr010 outputs, and traceability artifacts
   - Requirement-change handling: whether impact analysis/replan/revalidation is expected for requirement churn
...
4. Generate (internally) a prioritized queue of candidate clarification questions (maximum 5). Do NOT output them all at once. Apply these constraints:
   ...
   - Prioritize lifecycle-specific unknowns that affect component ownership or validation strategy before lower-impact wording clarifications.
...
5. Sequential questioning loop (interactive):
    ...
    - **Question writing quality (applies to every question, MC or short-answer):**
      ...
      - For lifecycle-related questions, explicitly anchor the question to one or more domains when relevant: Python PoC, lifecycle C++ components, Rust crates.
    - For multiple‑choice questions:
      ...
      - Recommendation reasoning should prefer constitution-aligned choices (reproducibility, traceability, impact-aware replanning) and explicit validation paths.
...
6. Integration after EACH accepted answer (incremental update approach):
    ...
    - Then immediately apply the clarification to the most appropriate section(s):
       ...
       - Lifecycle boundary clarification → Update constraints or requirements to state affected domain(s): `tools/ai_sdlc_poc`, `score/launch_manager`, `score/health_monitor`, Rust crates.
       - Validation-path clarification → Update acceptance criteria or quality gates with expected verification path (pytest, Bazel, Cargo) appropriate to the affected domain.
       - Evidence/provenance clarification → Update requirements/success criteria to include lockfile/evidence expectations where applicable.
...
7. Validation (performed after EACH write plus final pass):
   ...
   - Lifecycle consistency: no contradictory ownership statements across Python PoC, C++ lifecycle components, and Rust crate scope.
   - Validation consistency: if cross-domain impact is clarified, acceptance criteria reflect matching verification paths.
...
- Lifecycle alignment summary including:
   - Clarified domain ownership (Python PoC / C++ lifecycle components / Rust crates)
   - Clarified validation pathways (pytest/Bazel/Cargo)
   - Clarified evidence/provenance obligations (if in scope)
